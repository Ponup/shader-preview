...
vec2 sdGuy(in vec3 pos) {
    ...
    //head
    float d2 = sdElipsoid(h- vec3(0,0.28,0), vec3(0.15, 0.2,0.23));
    float d3 = sdElipsoid(h- vec3(0,0.28,-0.1), vec3(0.23, 0.2, 0.2));

    d2 = smin(d2, d3, 0.05);
    d = smin(d,d2, 0.15); // smooth two ellipsodies of the head
    ...
    //mouth
    d2 = sdElipsoid(h-vec3(0,0.15+3*pow(h.x, 2),0.15), vec3(0.1,0.04,0.2));
    d = smax(d, -d2,0.03);
